...
class Medarot {
    ...
    updatePosition() {
        ...
        } else if (this.state === 'ready_execute' && this.currentActionCharge) {
             currentMaxGauge = this.currentActionCharge;
        }
        ...
        if (this.state === 'action_charging') {
            positionXRatio = (this.team === 'team1') ? (progress * 0.5) : (1 - (progress * 0.5));
        } else if (this.state === 'idle_charging' || this.state === 'action_cooldown') {
            ...
            positionXRatio = 0.5;
             ...
             positionXRatio = (this.team === 'team1') ? 0 : 1;
        }
...
class GameManager {
                ...
                if (teamIndex === 0 && i === 0) {
                    ...
                    for (const slot of slotsToEquip) {
                        if (this.partsData[slot] && Array.isArray(this.partsData[slot])) {
                            ...
                            if (part) {
                                partsConfigForSet[slot] = part.id;
                            } else {
                                setComplete = false;
    ...
    gameLoop() {
        ...
        if (medarotToExecute) {
            return this.handleActionExecution(medarotToExecute);
        }

        const medarotToSelect = this.medarots.find(m => m.isReadyForSelection());
        if (medarotToSelect) {
            return this.handleActionSelection(medarotToSelect);
        }
    ...
    handleActionExecution(medarot) {
        ...
        this.pauseSimulation();
        this.prepareAndShowExecutionModal(medarot);
    }
    ...
    handlePartSelection(partKey) {
        ...
        this.activeMedarot.selectAction(partKey); // This clears previous target info
        ...
        if (attacker.selectedPartKey && attacker.parts[attacker.selectedPartKey]) {
            ...
            if (selectedPartInfo.category_jp === '射撃' && attacker.medal && attacker.medal.personality === 'ランダムターゲット') {
                ...
                if (enemies.length > 0) {
                    ...
                    const availableParts = Object.entries(randomEnemy.parts)
                                             .filter(([pK, pV]) => pV && !pV.isBroken)
                                             .map(([pK, _]) => pK);
                    if (availableParts.length > 0) {
                        const randomPartKeyOnEnemy = availableParts[Math.floor(Math.random() * availableParts.length)];
                        attacker.currentTargetedEnemy = randomEnemy;
                        attacker.currentTargetedPartKey = randomPartKeyOnEnemy;
                        console.log(`${attacker.name} (Player) targeted ${randomEnemy.name}'s ${randomPartKeyOnEnemy} with ${selectedPartInfo.name_jp}`);
                    } else {
                        // attacker.currentTargetedEnemy and PartKey are already null from selectAction
            ...
            console.warn(`${attacker.name} (Player) has selectedPartKey ${attacker.selectedPartKey}, but part info is missing in attacker.parts.`);
        }
